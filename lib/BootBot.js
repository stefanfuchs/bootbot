...
class BootBot extends EventEmitter {
  ...
  getUserProfile(userId) {
    ...
    const url = `https://graph.facebook.com/${this.graphApiVersion}/${userId}?fields=first_name,last_name,profile_pic&access_token=${this.accessToken}`;
    return fetch(url)
      .then(res => res.json())
      .catch(err => console.log(`Error getting user profile: ${err}`));
  }

  getUserProfileInstagram(userId) {
    // const url = `https://graph.facebook.com/${this.graphApiVersion}/${userId}?fields=first_name,last_name,profile_pic,locale,timezone,gender&access_token=${this.accessToken}`;
    const url = `https://graph.facebook.com/${this.graphApiVersion}/${userId}?fields=name,username,profile_pic&access_token=${this.accessToken}`;
    return fetch(url)
      .then(res => res.json())
      .catch(err => console.log(`Error getting user profile: ${err}`));
  }
  ...
  handleFacebookData(data) {
    ...
    data.entry.forEach((entry) => {
      ...
      entry.messaging.forEach((event) => {
        ...
        if (event.optin) {
        ...
        } else if (event.message && (event.message.text || event.message.is_deleted)) {
          ...
          console.log('Webhook received unknown event: ', event, JSON.stringify(event));
        }
      });
    });
  }
